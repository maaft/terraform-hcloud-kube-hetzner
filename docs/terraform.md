...
| <a name="input_custom_networking"></a> [custom\_networking](#input\_custom\_networking) | Configuration for custom networking overlay. | <pre>object({<br/>    enabled = bool<br/><br/>    static_nodes = object({<br/>      script_content      = string<br/>      interpreter         = optional(string, "/bin/bash")<br/>      timeout_seconds     = optional(number, 300)<br/>      max_retries         = optional(number, 3)<br/>      retry_delay_seconds = optional(number, 30)<br/>      output_file         = optional(string, "/tmp/custom-networking.json")<br/>    })<br/><br/>    autoscaler_nodes = optional(object({<br/>      enabled             = optional(bool, false)<br/>      script_content      = string<br/>      interpreter         = optional(string)<br/>      timeout_seconds     = optional(number)<br/>      max_retries         = optional(number)<br/>      retry_delay_seconds = optional(number)<br/>      output_file         = optional(string, "/tmp/custom-networking-autoscaler.json")<br/>    }))<br/>  })</pre> | <pre>{<br/>  "enabled": false<br/>}</pre> | no |
